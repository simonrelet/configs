function tp --description 'Teleport through folders'
  ...
  function tp_usage
    ...
    printf "  add <jump-site>  Add a new jump site with name <jump-site> for the current\n"
    printf "                   location. If one already exists with the same name, an error\n"
  ...
  function tp_add
    ...
    if [ $tmp != 2 ]
      set_color red
      printf "Give you jump site a name!\n\n"
      set_color normal
      tp_usage
      return 1
    end
